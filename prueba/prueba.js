...
const noVocals$$ = document.body.querySelector('#nameWithoutVocals');
const date$$ = document.body.querySelector('.date');
const days$$ = document.body.querySelector('#days');
const toDate = new Date(2023,2,20);

function calculateIntervalDays() {
    
    const currentDate = new Date(date$$.value);
    const diffTime = toDate - currentDate;
    const diffDays = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
    days$$.value = diffDays;
}

function removeVocalsAndSpaces() {
    let newText = inputName$$.value.replace(/ /g,'');
    noVocals$$.value = newText.replace(/[aeiou]/gi, '');
}
