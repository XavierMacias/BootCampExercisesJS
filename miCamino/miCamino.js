...
for(let i=0;i<miCamino.material.length;i++) {
  let mat = miCamino.material[i];
  let posicion = "Lo siguiente que debo llevar es";
  if(i == 0) {
    posicion = "Lo primero que debo llevar es";
  } else if(i == 1) {
    posicion = "Lo segundo que debo llevar es";
  } else if(i == 2) {
    posicion = "Lo tercero que debo llevar es";
  } else if(i == miCamino.material.length-1) {
    posicion = "Y por último debo llevar la";
  }
  console.log(`${posicion} ${mat.nombre} y es ${mat.descripcion}`);
}
...
const frase = miCamino.subiendoAlEverest[0][0].campoBase[0].campoI.campoII.campoIII.campoIV;
console.log(`${frase[0]}, ${frase[1]} ${frase[2]} ${frase[3]} ${frase[4]} ${frase[6]} ${frase[7]}`);

//7º: mostraremos el mensaje final guardado en la cima
console.log(`${frase[8].cima}`);
...
for(let i=0;i<miCamino.material.length;i++) {
  let mat = miCamino.material[i];
  for(let j=0;j<mat.imagenes.length;j++) {
    let img = mat.imagenes[j];
    if(j%2==0) {
...
let mediaAltura = 0;
for(let i=0;i<miCamino.cimas.length;i++) {
  let cima = miCamino.cimas[i];
  mediaAltura += cima.altitud;
}

console.log(`Media de las alturas de las cimas: ${mediaAltura/miCamino.cimas.length}`);